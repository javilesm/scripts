...
function grant_access() {
    ...
    while IFS=',' read -r username password address databases privileges; do
        ...
        if [[ -z $databases ]]; then
            echo "Se debe especificar al menos una base de datos para otorgar permisos de acceso al usuario $username desde $address."
            exit 1
        fi
        ...
        if ! [[ "$privileges" =~ ^(ALL PRIVILEGES|CREATE|ALTER|DROP|GRANT|SELECT|INSERT|UPDATE|DELETE)$ ]]; then
            echo "El nivel de acceso $privileges no tiene un formato válido para el usuario $username en las bases de datos $databases desde $address."
            exit 1
        fi
