...
function validate_config_file() {
  ...
  if [ ! -f "$CONFIG_PATH" ]; then
    echo "ERROR: El archivo de configuración de Dovecot no se puede encontrar."
    exit 1
  fi
  echo "El archivo de configuración de Dovecot existe."
}
# Función para ejecutar el configurador de Dovecot
function dovecot_config() {
  echo "Ejecutar el configurador de Dovecot..."
